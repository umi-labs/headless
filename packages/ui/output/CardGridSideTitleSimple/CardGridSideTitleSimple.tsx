...
import { Eye, Rocket } from "@phosphor-icons/react";

export default function CardGridSideTitleSimple({ data }: HeroWithMediaProps) {
  return (
    <section
      className={cn(
        "min-h-full w-full relative flex items-center justify-center py-10 md:py-32 overflow-visible bg-[#FAFAFA] ",
        data.topBuffer && "mt-10 md:mt-20 lg:mt-28 xl:mt-30",
      )}
    >
      {data.topBuffer && (
        <WaveSVG className="absolute -top-0 h-auto w-full -translate-y-full" />
      )}
      <div className="md:grid md:grid-cols-3 md:grid-rows-1 flex flex-col items-center justify-center w-full gap-10 md:gap-16 px-6 md:px-10 text-center relative z-10">
        <div className="flex flex-col items-center justify-center md:items-start md:justify-start place-self-start text-center md:text-start h-fit w-full gap-y-10 grid-flow-row-dense ">
          {data.separator && <EyebrowSVG className="" />}
          <h2>{data.title}</h2>
          <p>{data.content}</p>
          <div className="flex flex-col lg:flex-row lg:items-center items-start justify-start gap-6 w-full">
        ...
        <div
          className={cn(
            "grid grid-flow-row md:grid-cols-2 items-end justify-center w-full gap-9 col-span-2 lg:mt-20",
          )}
        >
          {data.features.map((feature, i) => (
            <Card feature={feature} index={i} />
          ))}
        </div>
      </div>
    </section>
...
const Card = ({
  ...
  feature: HeroWithMediaProps["data"]["features"][0];
  index: number;
}) => {
  return (
    <div
      id={`card_${index}`}
      className={cn(
        "flex flex-col items-center w-full gap-y-10 p-6 bg-white shadow-md h-full",
        feature.icon ? "justify-between" : "justify-center",
      )}
    >
      {feature.icon && (
        <div className="p-10 relative flex items-center justify-center size-48">
          <IconBackground
            className={cn(`absolute inset-0 z-0 size-full mr-2.5`)}
          />
          <IconSwitcher
...
export type IconType = "eye" | "rocket";
...
export type IconsSwitcherProps = {
  icon: IconType;
  weight: IconWeight;
} & React.HTMLAttributes<HTMLOrSVGElement>;

const IconSwitcher = (props: IconsSwitcherProps): React.ReactNode => {
  switch (props.icon) {
    case "eye":
      return <Eye {...props} />;
    case "rocket":
...
const WaveSVG = (props: React.HTMLAttributes<HTMLOrSVGElement>) => {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="1440"
    ...
    >
      <path
        d="M520.766 0.917101C390.85 1.72287 119.457 31.9171 0 46.9171V109.917H1440V86.9171C1185.38 80.9013 846.57 -1.10361 520.766 0.917101Z"
        fill="#FAFAFA"
      />
    </svg>
  );
