...
const ControlPropsSchema = z.object({
    ...
    type: z.enum(["text", "email", "password", "number", "tel", "url", "search", "date", "time", "datetime-local", "month", "week"]),
});

type ControlProps = z.infer<typeof ControlPropsSchema>;

function Control({ children, variant, type, ...props }: ControlProps) {
    return variant == "input" ? <input type={type} {...props} /> : variant == "textarea" ? <textarea {...props} /> : variant == "select" ? <select {...props} /> : null
}
