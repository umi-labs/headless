...
import { checkIfPackageInstalled } from "../utils/package";
...
export async function init(options: { yes: boolean }) {
  ...
  if (options.yes) {
    response = {
      ...
      api:
        (existingConfig as Config).aliases?.api || DEFAULT_CONFIG.aliases.api,
      ui: (existingConfig as Config).aliases?.ui || DEFAULT_CONFIG.aliases.ui,
    };
  } else {
    response = await prompts(
      configPrompts(existingConfig as Config, DEFAULT_CONFIG)
    );
  }
